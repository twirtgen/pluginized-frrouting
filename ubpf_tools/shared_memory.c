...
int init_memory_management(memory_pool_t *mem_pool, uint8_t *mem, size_t len) {
    ...
    mem_pool->size_of_each_block = 2100; /* TEST */
    mem_pool->num_of_blocks = len / 2100;
...
void destroy_memory_management(memory_pool_t *mp) {
    map_deinit(&mp->shared);
}
