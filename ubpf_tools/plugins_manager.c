...
int load_from_json(const char *file_path) {
    ...
    for (i = 0; i < nb_plugins; i++) {
        ...
        if (!(path_str = json_object_get_string(path))) return -1;
        ...
        if (jit) { // the most nested jit var is taken (override jit_all)
            if (add_plugin(path_str, (size_t) extra_mem_int, id_plugin_int,
                           type_enum, name_str, jit_bool, after_str != NULL ? after_str : NULL) == -1) {
                fprintf(stderr, "Unable to add plugin %s. Abort...\n", name_str);
                err++;
            }
        } else {
            if (add_plugin(path_str, (size_t) extra_mem_int, id_plugin_int,
                           type_enum, name_str, jit_all, after_str != NULL ? after_str : NULL) == -1) {
                fprintf(stderr, "Unable to add plugin %s. Abort...\n", name_str);
                err++;
            }
        }
...
__add_plugin_generic(const void *generic_ptr, int id_plugin, int type_plug, int type_ptr,
                     size_t len, size_t add_mem_len, const char *sub_plug_name, const char *after, uint8_t jit,
                     const char **err) {
    ...
    if (!plugin_ptr) {
        plugin = init_plugin(add_mem_len, id_plugin);
        if (!plugin) return -1;
...
int __add_plugin_ptr(const uint8_t *bytecode, int id_plugin, int type_plugin, size_t len,
                     size_t add_mem_len, const char *sub_plugin_name, const char *after, uint8_t jit,
                     const char **err) {
    return __add_plugin_generic(bytecode, id_plugin, type_plugin, ADD_TYPE_OWNPTR, len, add_mem_len,
                                sub_plugin_name, after, jit, err);
}

int __add_plugin(const char *path_code, int id_plugin, int type_plugin, size_t add_mem_len,
                 const char *sub_plugin_name, const char *after, uint8_t jit, const char **err) {
    return __add_plugin_generic(path_code, id_plugin, type_plugin, ADD_TYPE_FILE, 0, add_mem_len,
                                sub_plugin_name, after, jit, err);
}

int add_plugin(const char *path_code, size_t add_mem_len, int id_plugin, int type_plugin,
               const char *sub_plugin_name, uint8_t jit, const char *after) {
    const char *err;
    if (__add_plugin(path_code, id_plugin, type_plugin, add_mem_len, sub_plugin_name, after, jit, &err) == -1) {
        fprintf(stderr, "%s\n", err);
        return -1;
    }
    return 0;
        ...
        switch (rcvd_msg.plugin_action) {
            case E_BPF_ADD:
                if (__add_plugin_ptr(data_ptr, rcvd_msg.location, rcvd_msg.plugin_type,
                                     rcvd_msg.length, 0, rcvd_msg.name, rcvd_msg.after, rcvd_msg.jit, &str_err) < 0) {
                    err = 1;
                ...
                if (rm_plugin(rcvd_msg.location, &str_err) == -1) {
                    ...
                    strncpy(info.reason, str_err, strlen(str_err));
                        ...
                        __add_plugin_ptr(data_ptr, rcvd_msg.location, rcvd_msg.plugin_type,
                                         rcvd_msg.length, 0, rcvd_msg.name, rcvd_msg.after, rcvd_msg.jit, &str_err) <
                        0) {
                    err = 1;
                    strncpy(info.reason, str_err, strlen(str_err));
                }
